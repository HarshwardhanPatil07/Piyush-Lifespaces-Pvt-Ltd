...
// GET - Fetch reviews with filtering and sorting
export async function GET(request: NextRequest) {
  try {
    ...
    // Build query - include all reviews if admin, otherwise only approved
    ...
    // Select fields based on admin access
    const selectFields = includeAll ? '' : '-email -phone -updatedAt'
    ...
    const statsQuery = includeAll ? {} : { status: 'approved' }
    const allReviews = await Review.find(statsQuery)
    const stats = {
    ...
    const properties = await Review.distinct('property', statsQuery)
